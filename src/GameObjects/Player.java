...
public class Player extends MovingObject {
    ...
    @Override
    public void Destroy() {  // cuando se destruye la nave
        spawning = true;
        spawnTime.run(constans.SPAWNING_TIME);
        resetValues();
        gameState.subtractLife(); //para que reste una vida 
    }

    private void resetValues() { //para que el jugardo aparezca en el centro de la pantalla luego de ser destruído
        angle = 0;
    ...
    public void draw(Graphics g) {
        if (!visible) { //para hacer el efecto de parpadeo
            return;
        }
